...
class Idea(Resource):
    ...
    def post(self):
        ...
        if IdeaModel.find_by_title(data.title):
            return {"message": "An idea with title '{}' already exist.".format(data.title)},400
        print(data,"<<<data")
        idea = IdeaModel( **data)
        print(idea.json(),"<<<idea.json()")
...
class IdeaList(Resource):
    ...
    def post(self):
        ...
        if IdeaModel.find_by_title(data.title):
            ...
            return {'message': "An idea with idea_title '{}' already exists.".format(data.title)}, 400
        print(data,"<<<<data after found by title")
        idea = IdeaModel(**data) 
        print(idea.json(),"<<<<idea.json")
